...
use crate::position::coordinates::{idx, Coords, Row::*, CoordsVec, IdxCoordsVec, IdxCoords, CoordsVecEquivalent};
use crate::position::board::Board;
use crate::position::coup::Coup;


pub fn pawn_reachable_squares(board:&Board, square:&Coords, color:&Color) -> CoordsVec {
    ...
    let (square_rowidx, square_colidx) = square.to_colidx_rowidx();
    let mut in_reach = IdxCoordsVec::with_capacity(4);

    // One square forward 
    let next_square: IdxCoords = idxcoords!(square_colidx, square_rowidx + direction); // square in front of the pawn.
    let next_square_is_free: bool = board.square_is_free(&Coords::from_colrow_idx(&next_square));
    ...
    for colshift in [-1i8,1i8] {
        ...
        match board.color_of_piece_at(&Coords::from_colrow_idx(&potential_target_square)) {
    ...
    match (square.row, color) { 
        ...
        (R2, White) | (R7, Black) => {
            ...
            && board.square_is_free(&Coords::from_colrow_idx(&idxcoords!(square_colidx, square_rowidx + 2*direction)))
        ...
        (R5, White) | (R4, Black) => { 
            ...
            let last_move_colidx = idx(last_move.start.col);
