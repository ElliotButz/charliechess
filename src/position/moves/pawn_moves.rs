...
use crate::position::coordinates::{Square, Row::*, SquareVec, CoordsVec, Coords, SquareVecEquivalent};
...
pub fn pawn_reachable_squares(board:&Board, square:Square, color:Color) -> SquareVec {
    ...
    for colshift in [-1i8,1i8] {
        let potential_target_square = next_square + coords!((colshift, 0)) ;
        match board.color_of_piece_at(square!(potential_target_square)) {
